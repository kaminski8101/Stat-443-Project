...
1. We recommend the client to modify AFDP. 

2. In the future, it would be great if the client could expand their typical production range since the original typical production range did not accurately capture the range. 

3. Client should check with the scientists beforehand to check if any outliers are entered correctly. 
...
Extending description of variables 
...
ggplot(model, aes(x = gt_2015$TEY, y = gt_2015$AT), xlim = c(100,180)) + geom_jitter(width = 3, color = "navy") + theme_classic() + labs(x = "Total Energy Yield") + labs(y = "Ambient Temperature") + labs(title = "Figure 6: Relationship between Total Energy Yield & Ambient Temperature")
```
...
ggplot(model, aes(x = gt_2015$CO, y = gt_2015$AT), xlim = c(100,180)) + geom_jitter(width = 3, color = "navy") + theme_classic() + labs(x = "Carbon Monoxide") + labs(y = "Ambient Temperature") + labs(title = "Figure 7: Relationship between Carbon Monoxide & Ambient Temperature")
```
