...
all_bc_linear_mod_lm     <- lm( ( (CO^(.2)-1)/.2  ) ~ AT + AP + AH + AFDP + GTEP + TAT,
                                data = test_all)
typical_bc_linear_mod_lm <- lm( ( (CO^(.6)-1)/.6  ) ~ AP + AH + AFDP + GTEP + TAT + TEY + CDP,
                                data = test_typical)
high_bc_linear_mod_lm    <- lm( ( (CO^(1.9)-1)/1.9) ~ AT + AFDP + GTEP + TIT + TAT,
                                data =  test_high)
bc_pred_all <- predict(all_bc_linear_mod_lm, test_all, method="BoxCox")
bc_pred_typical <- predict(typical_bc_linear_mod_lm, test_typical, method="BoxCox")
bc_pred_high <- predict(high_bc_linear_mod_lm, test_high, method="BoxCox")
...
In order to determine which model was the most effective, we compared the RMSE of multiple linear regression, lasso, and decision tree models. Root Mean Squared Errors are the standard deviation of residuals. Residuals are a measure of the distance between the predicted and actual values. The point of calculating the RMSE is to measure how spread out these variables are. The rule of thumb is, the lower the RMSE, the better. 
...
all_bc_rmse     <- RMSE(test_all$CO, bc_pred_all)
typical_bc_rmse <- RMSE(test_typical$CO, bc_pred_typical)
high_bc_rmse    <- RMSE(test_high$CO, bc_pred_high)
...
RMSE_Table <- matrix(c(round(all_linear_rmse    ,digits = 4),
                       ...
                       round(all_bc_rmse        ,digits = 4),
                       round(typical_bc_rmse    ,digits = 4),
                       round(high_bc_rmse       ,digits = 4),
...
Based on our results, the following variables are the most sensitive process variables for the typical production range:

* NA
* NA

Based on our results, the following variables are the most sensitive process variables for the high production range:
