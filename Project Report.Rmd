...
#Linear log model
all_log_linear_mod <- train(
          form = log(CO) ~ . - NOX  - TIT - CDP - TEY ,
          data = gt_2015,
          method = "lmStepAIC",
          trControl = cv_5,
          nvmax = 10,
...
#Linear log model
typical_log_linear_mod <- train(
          form = log(CO) ~ . - NOX - TIT - AT,
          data = gt_2015_typical,
          method = "lmStepAIC",
          trControl = cv_5,
          nvmax = 10,
...
#Linear log model
high_log_linear_mod <- train(
          form = log(CO) ~ . - NOX - TEY - CDP,
          data = gt_2015_high,
          method = "lmStepAIC",
          trControl = cv_5,
          nvmax = 10,
...
plot(cooks.distance(all_log_linear_mod$finalModel))


plot(cooks.distance(typical_linear_mod$finalModel))
plot(cooks.distance(typical_log_linear_mod$finalModel))


plot(cooks.distance(high_linear_mod$finalModel))
plot(cooks.distance(high_log_linear_mod$finalModel))



which(cooks.distance(all_linear_mod$finalModel) > .05)
which(cooks.distance(all_log_linear_mod$finalModel) > .04)


which(cooks.distance(typical_linear_mod$finalModel) > .05)
which(cooks.distance(typical_log_linear_mod$finalModel) > .04)


which(cooks.distance(high_linear_mod$finalModel) > .05)
which(cooks.distance(high_log_linear_mod$finalModel) >.04)




which(cooks.distance(all_linear_mod$finalModel) > 4/nrow(gt_2015))
which(cooks.distance(all_log_linear_mod$finalModel) > 4/nrow(gt_2015))


which(cooks.distance(typical_linear_mod$finalModel) > 4/nrow(gt_2015_typical))
which(cooks.distance(typical_log_linear_mod$finalModel) > 4/nrow(gt_2015_typical))


which(cooks.distance(high_linear_mod$finalModel) > 4/nrow(gt_2015_high))
which(cooks.distance(high_log_linear_mod$finalModel) > 4/nrow(gt_2015_high))
```
